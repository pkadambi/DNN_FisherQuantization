...
def get_acc_for_gaussian_perturbed_logistic_model_MNIST(mu, sigma=.1, const_multiplier=1., record_tensorboard=False):
    ...
    bias1 = tf.get_variable('bias1',shape= (10), initializer = tf.random_normal_initializer(stddev=.1))
    ...
    if const_multiplier>0.:
        ...
        train_op = optimizer.apply_gradients(zip([tot_grads1, ce_grads[1]], [w, bias1]))
    else:
    ...
    if record_tensorboard:
        summary_writer = tf.summary.FileWriter('./logs/logistic_adam_v', sess.graph)
    ...
    for i in range(0, n_iters):
        ...
        _, l, acc, w_ = sess.run([train_op, loss, accuracy, w,], feed_dict={x: x_batch, y: y_batch, gamma:regularizer_const, w_pert:w_pert_})

        if record_tensorboard:
            summ, _, l, acc, w_ = sess.run([summary_op, train_op, loss, accuracy, w], feed_dict={x: x_batch, y: y_batch, gamma:regularizer_const, w_pert:w_pert_})
    ...
    w_pert_ = np.array(w_).reshape(784,10) + np.random.normal(mu, sigma, size = [784, 10])
    ...
    sess.run(tf.assign(w, w_pert), feed_dict={gamma:regularizer_const, w_pert:w_pert_})

    pert_acc = sess.run(accuracy, feed_dict={x: mnist.test.images, y: mnist.test.labels, gamma:regularizer_const, w_pert:w_pert_})
...
def get_acc_for_nonzero_gaussian_perturbed_two_layer_model_MNIST_diag(mu, sigma=.1, const_multiplier=1., record_tensorboard=False):
    import tensorflow as tf
    from tensorflow.examples.tutorials.mnist import input_data
    from tensorflow.python.ops import gradients_impl
    import numpy as np
    ...
    if const_multiplier>0.:
        ...
        tf.summary.histogram('diagonal_load1', diag_load_amt1)
        tf.summary.histogram('diagonal_load2', diag_load_amt2)
        tf.summary.histogram('ce_gradient1', reg_grad1)
        tf.summary.histogram('ce_gradient2', reg_grad2)
        tf.summary.scalar('loss_gamma', gamma)
...
def get_acc_for_gaussian_perturbed_logistic_model_MNIST_diag(mu, sigma=.1, const_multiplier=1., record_tensorboard=False):
    import tensorflow as tf
    from tensorflow.examples.tutorials.mnist import input_data
    from tensorflow.python.ops import gradients_impl
    import numpy as np
    ...
    if const_multiplier>0.:
        ...
        hvp1 = tf.multiply(fisher_diag, perturbation)
        # hvp1 = gradients_impl._hessian_vector_product(loss, [w], [perturbation])
        ...
        tf.summary.histogram('ce_gradient1_sq', tf.square(ce_grads_w1))
        tf.summary.scalar('loss_gamma', gamma)
        train_op_reg = optimizer.apply_gradients(zip([reg_grad1], [w]))
    else:
        print('NO REGULARIZATION')
        train_op_reg = tf.no_op()
    ...
    # exit()
    tf.summary.histogram('adam_v_t', vars[-1])
    tf.summary.histogram('weights1', w)
    tf.summary.histogram('pertweights1', w_pert)
    ...
    for i in range(0, n_iters):
        ...
        _, __, l, acc, w_ = sess.run([train_op, train_op_reg, loss, accuracy, w,], feed_dict={x: x_batch, y: y_batch, gamma:regularizer_const, w_pert:w_pert_})

        if record_tensorboard:
            summ, _, __, l, acc, w_ = sess.run([summary_op, train_op, train_op_reg,  loss, accuracy, w], feed_dict={x: x_batch, y: y_batch, gamma:regularizer_const, w_pert:w_pert_})
            summary_writer.add_summary(summ, i)
        lossval.append(l)
        accval.append(acc)
...
a, b = get_acc_for_nonzero_gaussian_perturbed_two_layer_model_MNIST(.1, .005, const_multiplier=0.00, record_tensorboard=False)
